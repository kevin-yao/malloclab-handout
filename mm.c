...
int mm_checkheap(int verbose) {
    uint32_t* bp = (uint32_t*) heap_listp;
	uint32_t headerSize;
	uint32_t footerSize;
	int prevFree = 0;
	int hfree;
	...
	if(verbose==1){
		if(!in_heap(bp-2)){
			printf("Prologue header is not in heap, add: %p\n", 
					(void*)(bp-2));
			return 1;
		}
		//check prologue header
		if((block_size(bp-2)!= OVERHEAD) || block_free(bp-2)){
			printf("Prologue header error! size: %u, free: %d\n",
                block_size(bp-2),  block_free(bp-2));
			return 1;
		}
		if(!in_heap(bp-1)){
			printf("Prologue footer is not in heap, add:%p\n",
					(void*)(bp-1));
			return 1;
		}
		//check prologue footer
		if((block_size(bp-1)!= OVERHEAD) || block_free(bp-1)){
			printf("Prologue footer error! size: %u, free: %d\n",
                   block_size(bp-1),  block_free(bp-1));
			return 1;
		}
		// check segregated free list
		int isError = checkFreeList();
		if(isError == 1){
			return isError;
		}
		
		while(block_size(bp)){
			headerSize = block_size(getHeader(bp));
			footerSize = block_size(getFooter(bp));
			hfree = block_free(getHeader(bp));
			ffree = block_free(getFooter(bp));
			//check bp in heap
			if(!in_heap(bp)|| !in_heap(bp+block_size(bp)-1)){
				printf("bp is not in heap, bp address: %p\n", (void*)bp);
				return 1;
			}
			//check header footer match
			if(get(getHeader(bp))!=get(getFooter(bp))){
				printf("Header and footer don't match!"
					"%p:header:[%u:%c] footer: [%u:%c]\n",(void*)bp, 
					headerSize, (hfree?'f':'a'), 
					footerSize, (ffree?'f':'a'));
				return 1;
			}
			/*
			 * Check whether block size is less than minimal size.
			 * Since header footer match has been checked, I only user 
			 * headersize to check
			 */
			if(headerSize < (MIN_BLOCK_SIZE/WSIZE)){
				printf("Header and footer size less than min at address: %p\n"
						, (void*) bp);
				return 1;
			}	  
			//check payload area alignment
			if(!aligned(bp+1)){
				printf("Allignment error at address: %p\n", 
				      (void*)(bp+1));
				return 1;
			}
			//check whether every free block is in free list
			if(hfree){
				if(!free_block_marked(bp)){
					printf("Free block not in free list at address: %p\n", 
				      (void*)(bp));
					return 1;
				}else{
					//sweep mark bit
			...
			//check whether there are consecutive free block
			if(prevFree && hfree){
				printf("Two consecutive free blocks at address: %p\n",
					  (void*)bp);
				return 1;
			}
			prevFree = hfree;
		...
		//check epilogue
		if(get(bp)!=EPILOGUE){
			printf("Epilogue header error at address: %p! Epilogue: %u\n",
					(void*)bp, get(bp));
			return 1;
		}
		//check whether epilogue is in heap
		if(!in_heap(bp)){
			printf("bp is not in heap\n");
			return 1;
		}
	}else if(verbose==2){/*print all invariant*/
		printf("Prologue header: %u, Address: %p\n", get(bp-2), 
				(void*)(bp-2));
		printf("Prologue footer: %u, Address: %p\n", get(bp-1), 
				(void*)(bp-1));
		...
		while(block_size(bp)){
			...
			printf("%p: header: [%u:%c], footer:[%u:%c]\n", (void*)bp,
				headerSize, (hfree?'f':'a'), footerSize, (ffree?'f':'a'));
			if(hfree){
				printf("%p: prev: %d, %p: next: %d\n", (void*)get_prev_ptr(bp), 
					getOffset(get_prev_ptr(bp)), (void*)get_next_ptr(bp), 
					getOffset(get_next_ptr(bp)));
			}
			bp = block_next(bp);
		}
		printf("Epilogue: %u,  Address: %p\n", get(bp), (void*)bp);
	}
